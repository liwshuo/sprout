// services/calendar-service.js —— 日历四源聚合服务
...
//   date,       // 'YYYY-MM-DD'（归属日）
//   ts,         // 当天 0 点时间戳（排序/定位用）
//   type,       // 'record' | 'schedule' | 'todo' | 'reading'
//   title,      // 主标题
//   subtitle,   // 副标题（备注 / 地点 / 页码等）
//   time,       // 时间段（课表专用，如 '16:00-17:00'）
//   color,      // 事件主题色（= 类型色，驱动圆点与卡片色条）
//   typeLabel,  // 类型中文名（成长记录 / 课外班 / 待办 / 阅读打卡）
//   sourceId,   // 上游文档 uuid
//   raw,        // 原始文档
//   ...extras   // record: category/categoryColor/mood；schedule: scheduleType；todo: category/done；reading: mood
// }
//
...
// 圆点固定顺序：成长记录(橙) → 课外班(蓝) → 待办(紫) → 阅读打卡(绿)，最多 4 个
const DOT_ORDER = ['record', 'schedule', 'todo', 'reading'];
...
/** todos → CalendarEvent（第三源；仅取带 dueDate 的待办，按截止日落点） */
function _toTodoEvent(t) {
  const ts = dateUtil.startOfDay(new Date(String(t.dueDate).replace(/-/g, '/')));
  return {
    date: t.dueDate,
    ts,
    type: 'todo',
    ...
    subtitle: t.remark || '',
    time: '',
    color: eventTypeColor('todo'),
    typeLabel: EVENT_TYPE_LABELS.todo,
    category: t.category || '',
    mood: t.done ? '✅' : '',
    done: !!t.done,
    sourceId: t.uuid,
...
/** reading_logs → CalendarEvent（第四源；title 取书名） */
...
/**
 * 拉取「当月」四源事件并聚合为 CalendarEvent[]。
 * 四源并发拉取；课表按 weekly 规则展开到当月每一天。
 *
...
async function fetchMonthEvents(childId, year, month) {
  ...
  // 四源 + 书名映射并发拉取（均走 listAllPaged 破 20 条上限）
  const [records, scheduleItems, todos, readingLogs, books] = await Promise.all([
...
/**
 * 计算某天日历圆点颜色（最多 4 个，按 record→schedule→todo→reading 去重）。
