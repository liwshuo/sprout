...
- **四源综合视图**：日历同时聚合展示 **兴趣班课程 / 待办 / 成长记录 / 阅读打卡** 四类事项（每天最多 4 个圆点：橙=成长记录、蓝=兴趣班、紫=待办、绿=阅读打卡），点击某天在下方以统一事件卡片展开当天各类详情。**课表源仅展示「兴趣班」（`type !== 'school'`）；学校常规课（`type === 'school'`）不进日历，避免每天铺满——完整周课表见课表 Tab。**
- **图例**：成长记录（橙）/ 兴趣班（蓝）/ 待办（紫）/ 阅读打卡（绿）分类色说明。
- **当日综合安排**：选中日期下方按「记录→课表→待办→阅读」顺序展示当天事件卡片（时间轴样式）；待办已完成时标题划线置灰并显示 ✅。**待办卡片在日历上只读——点击仅查看详情，勾选完成/编辑/删除统一回到「待办」Tab。**
...
- **三分区**：在读 / 想读 / 已读，分段切换并显示各区数量；顶部「藏书 / 在读 / 读完」概览统计条（含系列分册）。
- **书籍卡片**：暖橙马卡龙大封面卡片 + 书名 + 作者 + 状态徽章 + 打卡次数/时长 + 进度环。进度全部由 `BookShelfService` 聚合派生，页面不直接读书籍进度字段。
- **书型（`bookType`）**：新增三分类，决定打卡交互形态——
  - **绘本 `picture`**：默认「读完整本」一键打卡（无需填页码），可切到「只读了一部分」填页码区间；总页数 ≤ 80 的书也走一键。
  - **章节书 `chapter`**：打卡时以 chips 选「读到第几章」（已知总章数时展示各章、标注已读、默认选中下一章；无章节元信息时手填章节文本）；选到最后一章视为读完。
  - **自由阅读 `free`**：起始页自动带上次进度（上次结束页 + 1），只需填结束页。历史书无 `bookType` 时按「有 `totalChapters` → 章节书，否则自由阅读」推断，向后兼容。
- **添加书籍**：右下角「＋」弹出**添加方式选择**（精选书库 / 手动录入 / 扫码添加 / 新建系列）。
  - **精选书库**（已落地）：点「📚 精选书库」进入书库浏览页，分龄挑书一键加入书架（自动按书库 type 映射 `bookType`），详见 §4.8。
  - **手动录入**：底部弹层手填书名（必填）+ 作者（选填）+ **书型 segmented**；绘本/自由阅读可填总页数，章节书可填「共几章」或逐行填章节名（填了以章节名为准）；可勾选**「这本书属于某个系列」（方案 B）**，选择已有系列或「＋ 新建系列」（填系列名 + 总册数），并可指定册序（留空自动排序）。
...
- **系列书面板**（套书，已落地）：书架把同一 `seriesUuid` 的书聚合为**系列卡片**（三层叠层封面 + 右上「系列」徽标 + **「已读 x / 共 x 册」文字 + 百分比 + 进度条**）；点开进入系列面板，按 `seriesIndex` 升序列出各分册（册序 · 书名 · 状态角标 · 打卡），面板内可直接打卡（打卡后面板保持打开并刷新）与「＋ 添加分册」（方案 A：分册序号由系列上下文自动决定）。
  - **系列进度口径**：「共 x 册」优先取 `series.totalVolumes`（运营声明），未设置时回退当前已归属分册数；「已读 x」由分册 `status==='done'` **派生**，不冗余存储已读册数。
  - **「读完这册」**：系列分册打卡弹层提供「读完这册」勾选，勾选后该册标记读完、系列已读册数自动 +1；因已读数由派生得出，**重复「读完这册」不会重复计数（幂等）**。
- **阅读打卡**：`ReadingCheckinSheet` 按书型分支记录章节/页码、备注；显式「读完整本 / 读完这册 / 章节读到末章」通过 `markDone` 把状态强制置 done，其余按页/章派生（want → reading → done）在写侧判定。
...
- **勾选完成（方案 B：轻提示不打断）**：圆形勾选圈一键切换完成 / 未完成（乐观更新，先本地翻转再落库）；顶部完成进度条展示 `已完成 / 总数`。勾选为完成后**不再弹确认框**，改为底部短暂 toast「✅ 已完成！」并附可点的「记录成长 →」，约 2.6s 自动消失；点「记录成长 →」才进入已预填标题、备注、分类与完成日期的成长记录页补充图片/心情后保存。成长记录通过 `sourceType='todo'` + `sourceTodoId` 关联来源，待办通过 `convertedRecordId` 反向关联；保存由 `childShare.convertTodoToRecord` 在服务端事务内完成待办校验、确定性记录写入及关联回写，保证并发场景只产生一条记录。删除关联成长记录会清理待办侧关联，以便后续重新记录；仅取消待办完成状态不会删除已生成记录。
- **完成后自动记录成长（可选开关）**：「我的 → 设置」提供「完成待办后自动转为成长记录」开关（本地偏好 `utils/settings.js`，默认关闭）。开启后勾选完成即**静默**调用 `childShare.convertTodoToRecord` 生成一条成长记录（幂等，不重复创建），toast 文案变为「✅ 已完成并记录成长」且不再展示「记录成长 →」。
...
- **数据封装**：`utils/todo.js` 基于 `utils/db.js` 通用 CRUD 二次封装（`listAll`/`listByCategory`/`create`/`update`/`toggleDone`/`markConverted`/`convertToRecord`/`remove`），并导出 `TODO_CATEGORIES`/`categoryMeta` 展示令牌；本地使用偏好读写收敛在 `utils/settings.js`。
...
- **课外班/课程**：`schedule_items` 的周期规则（当前落地 weekly；biweekly/monthly/once 属 P1/P2）按展示月份**动态推算成具体日期，不落库**（`date.expandWeeklySchedule`）。**仅聚合「兴趣班」（`type !== 'school'`）；学校常规课（`type === 'school'`）在聚合前被过滤，不进日历。**
