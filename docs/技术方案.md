...
> 版本：**v1.5**　|　撰写：李硕　|　更新时间：2026-09-10
> 状态：技术设计评审稿（已对齐 2026-09-10 方案校准决策，见 `docs/DECISION_LOG_20260910.md`）
> **双端策略（路径 A）**：本技术方案（v1.5）**仅覆盖 Flutter App 端**（Local-First，本机 SQLite + iCloud / Android Auto Backup），为本人使用的"离线可用 + AI 润色 + 重度分析"高级版。**全家主入口小程序端**架构独立，完整设计见 `docs/miniprogram_architecture_design.md`，数据在 CloudBase（CloudBase 云数据库/云存储/云函数），双端 V1~V1.5 数据不打通共享，业务口径 100% 对齐 `docs/AGGREGATION_RULES.md`。双端打通共享列入 V3 远期目标。
> **V1 范围原则（本次校准）**：V1 = "能自用"的最小闭环——日常/阅读随手记 + 日历回看 + **本地结构化模板周报（零配置、无需 AI Key、无需后端）**。**语音快速记录、活动计时器降级为 P1**（M3 再做）；**Flutter 端扫码为次要入口**（默认搜书名/手填）；**AI 周报润色、语音语义解析、套书整套聚合进度均后置 V2**；**套书 V1 已实现系列书面板展示 + 分册管理，整套聚合进度 V2 再做**。V1 即接**系统级自动备份**（iCloud Drive / Android Auto Backup），课表补**单日停课简化版 excludedDates[]**，周报补**空周不生成**；本地长图轻分享与独立 CourseExceptions 集合（调课/改时段）列入 V2。里程碑（§10）与产品文档 §7 附表口径统一。
> 架构基线：Flutter + Riverpod + drift(SQLite)，领域/数据层与 UI 解耦；`StatefulShellRoute.indexedStack` 承载 4 Tab；书架进度统一由 `ReadingLogs` 聚合派生（`Books` 不冗余存进度）；全表预留 `uuid`/`updatedAt`/`isDeleted` 同步字段（V3 同步复用）。
...
| 模块 | V1/P1/V2 | 说明 |
| --- | --- | --- |
| 课表管理 | V1 | 维护孩子每周固定课程（学校课表 + 课外班，支持周期规则与学期范围）；单日停课 V1 简化为 schedule_items.excludedDates[]；独立 CourseExceptions 集合调课/改时段列入 V2 |
| 日常事项记录 | V1 | 记录晚上 / 周末的日常点滴，支持文字、**多图**、**标签**、心情（家长观察到的孩子心情） |
| 阅读记录 | V1 | 书单管理 + 进度打卡（绘本"读完/未读完+重复次数" / 页数 / 章节）+ 阅读时长统计（Books / ReadingLogs 拆分） |
| **书架** | V1 | 仿实体书架，按 在读 / 想读 / 已读完 分区可视化；**V1 已实现系列书面板展示 + 分册管理（叠层封面/系列面板 UI）**，整套聚合进度条（已读册数÷totalVolumes）列入 V2 |
| 日历视图 | V1 | 以日历为入口浏览所有维度的历史记录 |
| 周报 | V1 | 每周后台自动汇总本周记录，**本地结构化模板拼装成周报**（零配置、无需 AI Key）；**空周不生成**；长图导出分享列入 V2（等 AI 润色文案 + editedText 做完后） |
...
| **扫码录入** | **双端策略分离** | **Flutter 端次要入口**：扫 ISBN 经 Open Library 拉取书目，中文书/未命中/无网络/超时一律跳手填；默认主路径为搜书名/手填。**小程序端主入口**：扫 ISBN 走探数（主源，中文书目命中率高）+ Google Books 兜底；手填兜底 |
...
> **字段口径补充**：`Books.progress_mode` ∈ {`twoState`(绘本"读完/未读完"+重复次数)、`page`(页数环)、`chapter`(章节环)}；`ReadingLogs.is_finished` 标记该次打卡是否"读完一遍"，绘本进度 = 是否已有 `is_finished` 记录 + 打卡次数。`DailyRecords.mood` / `ReadingLogs.mood` 记录的是**家长观察到的孩子心情**（周报"情绪"维度即基于此）。`ReadingLogs.source` / `DailyRecords.source` 枚举保留 `manual`/`voice`/`timer` 以向前兼容，**V1 仅产生 `manual`**（voice/timer 为 P1）。**BOOK_STATUS 枚举统一口径为 `want/reading/done`（不使用 finished，见 docs/AGGREGATION_RULES.md）**。`Series.totalVolumes` 与套书聚合进度 **V1 不展示聚合进度条**，但系列书面板展示 + 分册管理 + 叠层封面 UI 已落地小程序端；整套聚合进度（已读册数÷totalVolumes）列入 V2。

### 4.2 表结构（drift `Table` 定义）

| 表 | 关键字段 | 用途 |
| --- | --- | --- |
| `Series`（套书） | `name` / `totalVolumes` / `sortOrder` | 套书主表（**V1 已落地「系列书面板 + 分册管理 + 叠层封面 UI」（小程序端）**，整套聚合进度条列入 V2）。V2 套书"已读完册数"实时聚合 `count(Books where seriesId=X and status='done')`，进度 = 已读完册数 ÷ `totalVolumes`，不冗余存字段 |
| `Books`（书单，重构） | `title` / `author` / `cover` / `isbn` / `status`(`want`/`reading`/**`done`**) / **`progressMode`(twoState/page/chapter)** / `totalPages` / `totalChapters` / `seriesId`(FK, nullable) / `seriesIndex` / `sortOrder` / `gender` / `gradeOverride` | 书单主表，支撑书架三分区、进度展示、封面。**`progressMode`**：绘本(无页无章)=`twoState`（读完/未读完 + 重复次数）、有总页数=`page`（页数环）、无页有章=`chapter`（章节环）。**`status` 自动跃迁规则**：新建默认 `want`；首次追加 `ReadingLog` 自动 `want→reading`；标记读完或手动置 `done`；**家长手动改 `status` 优先级最高**。`Books` **不存"当前已读页/章"**，进度一律由 `ReadingLogs` 聚合派生（见 §5.7） |
...
| `ScheduleItems`（重构） | `courseName` / `type`(school/extra) / `location` / `teacher` / `weekday`(1–7) / `recurrence`(weekly/biweekly/monthly/**once**) / `startTime` / `endTime` / `startDate` / `endDate` / **`excludedDates`**(JSON 字符串数组，存停课日期 YYYY-MM-DD，V1 简化版) | 学校课表 + 课外班，支持每周 / 隔周 / 每月 / **单次(once)** 周期与学期范围。**`once`=单次课**：不按周期循环展开，仅在 `startDate`（＝当天）落一次课程实例。**每条仅存单个 `weekday`**；"课外班多选周几"落库时**拆成多行**（每个 weekday 一行，共享 courseName / 时段等），不引入 `weekdays[]` 列。V1 单日停课走 excludedDates；独立 CourseExceptions 集合调课/改时段列入 V2。 |
| `CourseExceptions`（单日例外，**V2**） | `scheduleItemId`(FK) / `exceptionDate`(某天) / `type`(cancel/reschedule) / `newStartTime` / `newEndTime` / `newCourseName`(改上别的课时) | 课表**单日调课/改时段/改课例外**（V2 实现）。V1 先做 excludedDates 简化停课（见 ScheduleItems 表）；V2 再升级为独立集合，`cancel`=当天该课不展示（请假/节假日），`reschedule`=当天改时段/改课。**仅覆盖指定日期、不改动周期规则本身**；日历展开课程块时先按 `recurrence` 生成，再套用该日 `CourseExceptions`（置灰/改签标注），避免"显示有课实际没上" |
| `WeeklyReports` | `weekStart` / `weekEnd` / `summary`(**V1 本地模板结构化 JSON**) / `aiText`(**V2 LLM 润色原文，V1 为空**) / `editedText`(家长编辑版) / `status`(draft/published) / `dailyCount` / `readingCount` / `readingMinutes` / `extraClassCount` / `activeDays` / `generatedAt` | 周报快照，冗余统计字段避免重复计算。**V1 展示文本来自 `summary` 本地模板拼装**；`aiText` 仅在 V2 开启 AI 润色后写入（并支持"恢复原版"），`editedText` 存家长手改内容。空周不生成，去重幂等键 childId+weekStart。 |
| `Child`（孩子档案，V1 多孩子，扩展字段） | `name`(必填) / `birthDate`(**nullable，可后补**) / `avatarPath`(nullable) / **`gender`(boy/girl/unknown)** / **`gradeOverride`(String nullable，手动覆盖年级)** / **`sortOrder`(Int)** | 孩子基本信息。V1 支持**多孩子档案**（与小程序端对齐，见决策日志冲突 1）；`birthDate` 缺失时周报年龄/称呼降级为"孩子 / 宝贝"；`gender`/`gradeOverride`/`sortOrder` 新增字段同步 Flutter schemaVersion 升至 3。 |
